...
// Taking mu as a rowvec:
inline arma::vec dmvnorm(const arma::mat &x, const arma::rowvec &mu,
                         const arma::mat &S, const bool log_p = false) {
  arma::uword n = x.n_rows, m = x.n_cols;
  double det_S = arma::det(S);
  arma::mat S_inv = S.i();
...
inline arma::mat rmvnorm(const arma::uword n, const arma::rowvec &mu,
                         const arma::mat &S) {
  arma::uword m = S.n_cols, i, j;
  arma::mat result(n, m);
  for (i = 0; i < n; ++i) {
    for (j = 0; j < m; ++j) {
      result(i, j) = Rcpp::rnorm(1, 0.0, 1.0)(0);
    }
  }
  result = result * arma::chol(S);
  for (i = 0; i < n; ++i) {
